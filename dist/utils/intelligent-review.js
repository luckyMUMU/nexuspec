...
exports.generateReviewSuggestions = generateReviewSuggestions;
exports.formatSuggestion = formatSuggestion;
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
async function loadGraphData(namespacePath) {
...
function analyzeCompatibility(proposal, target) {
...
function analyzeImpact(proposal, target, graphData) {
    if (!graphData) {
        return 'medium';
    }
    // 分析依赖关系
    ...
    // 查找依赖此服务的其他服务
    const dependentServices = new Set();
...
function generateSuggestion(proposal, target, compatibility, impact) {
...
function calculateConfidence(compatibility, impact) {
...
async function generateReviewSuggestions(config, proposals) {
