...
// 验证契约兼容性
async function validateContractCompatibility(oldContractPath: string, newContractPath: string): Promise<boolean> {
  try {
    // 尝试从缓存获取契约内容
    let oldContractContent = cache.get(`contract:${oldContractPath}`);
    let newContractContent = cache.get(`contract:${newContractPath}`);

    // 如果缓存中没有，则读取文件
    if (!oldContractContent) {
      oldContractContent = await fs.readFile(oldContractPath, 'utf8');
      cache.set(`contract:${oldContractPath}`, oldContractContent);
    }

    if (!newContractContent) {
      newContractContent = await fs.readFile(newContractPath, 'utf8');
      cache.set(`contract:${newContractPath}`, newContractContent);
    }

    const oldContract = yaml.load(oldContractContent) as any;
    const newContract = yaml.load(newContractContent) as any;
...
validateCommand
  ...
  .action(async (name, options) => {
    try {
      await monitoring.measureExecutionTime('validate_contract', async () => {
        // 读取配置文件
        ...
        // 找到契约文件
        const oldContractPath = path.join(namespacePath, 'contracts', serviceName, contractType, `${options.oldVersion}.yaml`);
        const newContractPath = path.join(namespacePath, 'contracts', serviceName, contractType, `${options.newVersion}.yaml`);
        ...
        if (!await fs.exists(newContractPath)) {
        ...
        if (isCompatible) {
          ...
          const newContract = yaml.load(await fs.readFile(newContractPath, 'utf8')) as Contract;
          newContract.backward_compatible = true;
          await fs.writeFile(newContractPath, yaml.dump(newContract));
      ...
      handleError(error, 'validating contract');
    }
  });
...
validateCommand
  ...
  .action(async () => {
    try {
      await monitoring.measureExecutionTime('validate_all', async () => {
        // 读取配置文件
        const configPath = path.join(process.cwd(), '.nxsp', 'config.yaml');
        if (!await fs.exists(configPath)) {
          console.log('Error: .nxsp/config.yaml not found. Please run nxsp init first.');
        ...
        const serviceContractPath = path.join(namespacePath, 'contracts', serviceName);
        if (!await fs.exists(serviceContractPath)) {
          console.log('No contracts found for this service');
          process.exit(0);
        }

        // 遍历所有契约类型
        const contractTypes = await fs.readdir(serviceContractPath);
        ...
        for (const contractType of contractTypes) {
          const contractTypePath = path.join(serviceContractPath, contractType);
          if (await fs.stat(contractTypePath).then(stats => stats.isDirectory())) {
            // 遍历所有契约版本
            const contractFiles = await fs.readdir(contractTypePath);
            ...
            for (let i = 1; i < versions.length; i++) {
              ...
              const oldContractPath = path.join(contractTypePath, `${oldVersion}.yaml`);
              const newContractPath = path.join(contractTypePath, `${newVersion}.yaml`);
      ...
      handleError(error, 'validating all contracts');
    }
  });
