...
class ServiceDiscovery {
  ...
  private currentIndex: Map<string, number> = new Map(); // 用于轮询负载均衡
   ...
   * @returns 实例ID
   */
  registerService(serviceName: string, instanceUrl: string): string {
   ...
   * @returns 服务实例URL
   */
  getServiceInstance(serviceName: string): string | null {
    const service = this.services.get(serviceName);
    if (!service) {
      return null;
    }
